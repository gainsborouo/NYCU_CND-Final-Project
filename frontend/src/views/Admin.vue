      ...
      <div class="max-w-6xl mx-auto">
        ...
        <div v-else>
            ...
            <div class="flex items-center gap-4 text-sm">
              ...
              <div>
                ...
                <span class="text-cyan-400 ml-1">{{
                  usernames[document.creatorId] || document.creatorId
                }}</span>
              </div>
              ...
              <div v-if="document.currentReviewerId">
                ...
                <span class="text-cyan-400 ml-1">{{
                  usernames[document.currentReviewerId] ||
                  document.currentReviewerId
                }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            >
              <h3 class="text-lg font-bold mb-4 text-gray-200">
                Document History
              </h3>
              <div
                class="bg-gray-800 rounded-lg p-6 border border-gray-700 relative flex-1 flex flex-col"
              >
                ...
                >
                  ...
                  <div class="relative min-h-full">
                    ...
                    <div
                      class="absolute left-2 w-0.5 bg-gray-700"
                      :style="{
                        top: '0',
                        height: documentHistory.length
                          ? 'calc(100% - 16px)'
                          : '100%',
                      }"
                    ></div>
                    ...
                    <!-- Timeline items -->
                    <div
                      v-for="(historyItem, index) in documentHistory"
                      :key="historyItem.id"
                      class="relative pl-10 pb-8"
                    >
                      <!-- Timeline node -->
                      <div
                        :class="getActionNodeClass(historyItem.action)"
                        class="absolute left-2 w-6 h-6 rounded-full flex items-center justify-center transform -translate-x-1/2 border-4 border-gray-800 z-10"
                      ></div>
                      ...
                      <div class="bg-gray-700 rounded-lg p-4">
                        ...
                        <div class="flex justify-between items-start">
                          <div>
                            <span
                              :class="getActionBadgeClass(historyItem.action)"
                              class="px-2 py-1 rounded-full text-xs inline-block mb-2"
                            >
                        ...
                        <div class="text-sm mb-2">
                          <span class="font-semibold text-cyan-400">{{
                            usernames[historyItem.reviewer_id] ||
                            historyItem.reviewer_id
                          }}</span>
                          <span class="text-gray-300">
                            {{ getActionDescription(historyItem.action) }}</span
                          >
                        </div>

                        <!-- Status change info -->
                        <div
                          v-if="historyItem.new_document_status"
                        ...
                        >
                          ...
                          <span
                            :class="
                              getStatusBadgeClass(
                                historyItem.new_document_status
                              )
                            "
                        ...
                        <div
                          v-if="
                            ...
                            historyItem.rejection_reason !== '0'
                          "
                          class="mt-3 p-3 bg-gray-800 rounded text-sm text-red-300 border-l-2 border-red-500"
                        >
                          {{ historyItem.rejection_reason }}
                        </div>
                      </div>
            ...
            <div class="lg:col-span-7 flex flex-col" style="min-height: 500px">
              ...
              <div
                class="bg-gray-800 rounded-lg p-6 border border-gray-700 flex-1"
              >
                <form @submit.prevent="updateDocument">
                  <div class="space-y-6">
                    ...
                    <div v-if="updatedDocument.status === 'pending_review'">
                      ...
                      <div
                        v-else
                        class="mt-3 p-4 bg-yellow-900/30 border border-yellow-700 rounded-lg"
                      >
                        <p class="text-yellow-300 text-sm flex items-center">
                          ...
                          No reviewers available for this realm. Please add
                          reviewers to be able to assign them.
                        </p>
                      </div>
                    </div>
                    ...
                    <div
                      ...
                      "
                      class="p-3 rounded-lg text-sm mt-4"
                    >
                      {{ updateMessage }}
                    </div>
                  </div>
...
export default {
  ...
  setup(props) {
    ...
    const getStatusClass = (status) => {
      ...
      return (
        statusClasses[status?.toLowerCase()] || "bg-gray-600 text-gray-100"
      );
    };
    ...
    const getStatusBadgeClass = (status) => {
      ...
      return (
        statusClasses[status?.toLowerCase()] || "bg-gray-700 text-gray-100"
      );
    };
    ...
    const getActionBadgeClass = (action) => {
      ...
      return (
        actionClasses[action?.toLowerCase()] || "bg-gray-700 text-gray-100"
      );
    };
    ...
    const fetchDocumentHistory = async () => {
      try {
        const response = await documentService.getDocumentReviewHistory(
          props.id
        );
    ...
    const fetchReviewers = async (realmId) => {
      ...
      try {
        ...
        const reviewersArray = Object.entries(response).map(
          ([id, username]) => ({
            id,
            username,
          })
    ...
    const updateDocument = async () => {
      ...
      if (
        updatedDocument.value.status === "pending_review" &&
        !updatedDocument.value.currentReviewerId
      ) {
        reviewerError.value =
          "A reviewer is required when setting status to Pending Review";
